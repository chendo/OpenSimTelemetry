...
async fn test_telemetry_stream_with_field_filter() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri("/api/telemetry/stream?fields=speed,rpm")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    if let Ok(Some(text)) = result {
        ...
        if let Some(data_line) = text.lines().find(|l| l.starts_with("data:")) {
            ...
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(json_str) {
                // Should have speed and rpm
                assert!(parsed.get("speed").is_some(), "Filtered response should include speed");
                assert!(parsed.get("rpm").is_some(), "Filtered response should include rpm");
                // Should NOT have gear or throttle (not requested)
                assert!(
                    parsed.get("gear").is_none(),
                    "Filtered response should NOT include gear"
                );
                assert!(
                    parsed.get("throttle").is_none(),
                    "Filtered response should NOT include throttle"
                );
